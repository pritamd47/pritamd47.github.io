...
To perform these visualizations, we can get away with reducing the ~1.8TB dataset into two datasets, one of the mean and the other standard deviation across the ensemble members. This can be done using a combination of [xarray](https://docs.xarray.dev/en/stable/), and [Intake-ESM](https://intake-esm.readthedocs.io/en/stable/). This articles does not discuss this operation in detail, rather, the reader is guided towards [Project Pythia's CESM Lens2 cookbook](https://projectpythia.org/cesm-lens-aws-cookbook/notebooks/example-workflows/key-figures.html) for useful information on how to go about doing this operation.
